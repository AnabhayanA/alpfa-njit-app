...
export default function AnimatedALPFAMark({ progress, size }: Props) {
  const fillOpacity = progress.interpolate({
    inputRange: [0, 0.72, 0.94, 1],
    outputRange: [0, 0, 0.55, 1],
    extrapolate: 'clamp',
  });
  ...
  // Native gets the true SVG stroke-draw. Web keeps a safe reveal because
  // animated react-native-svg path props can crash the browser build.
  if (Platform.OS === 'web') {
    ...
    return (
      <View pointerEvents="none" style={[styles.stage, { width: size, height: size }]}>
        <Animated.View style={[styles.stage, { width: size, height: size, opacity: progress, transform: [{ scaleY: reveal }] }]}>
  ...
  return (
    <View pointerEvents="none" style={[styles.stage, { width: size, height: size }]}>
      <Svg width={size} height={size} viewBox="0 0 500 500">
        ...
        />
        <AnimatedPath d={ALPFA_RED_PATH} fill="#E02125" opacity={fillOpacity} />
      </Svg>
    </View>
  );
}
