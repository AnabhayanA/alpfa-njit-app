...
export default function ALPFALoadingScreen({ onAnimationComplete }: Props) {
  ...
  const alpfaMark = useRef(new Animated.Value(0)).current;
  const alpfaDetails = useRef(new Animated.Value(0)).current;
  ...
  useEffect(() => {
    const animation = Animated.sequence([
      ...
      Animated.delay(350),
      Animated.timing(alpfaMark, { toValue: 1, duration: 850, easing: Easing.out(Easing.cubic), useNativeDriver: false }),
      Animated.timing(alpfaDetails, { toValue: 1, duration: 520, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
      Animated.delay(1100),
      Animated.parallel([
  ...
  }, [alpfaDetails, alpfaMark, onAnimationComplete, opacity, scale, translateY]);

  const alpfaY = alpfaDetails.interpolate({ inputRange: [0, 1], outputRange: [-12, 0] });

  return (
    <View style={styles.root}>
      <Animated.View style={[styles.composition, { width: lockupWidth, height: lockupWidth * 0.92, opacity, transform: [{ translateY }, { scale }] }]}>
        <View style={styles.alpfaStage}>
          <AnimatedALPFAMark progress={alpfaMark} size={lockupWidth} />
          <Animated.View style={[StyleSheet.absoluteFillObject, { opacity: alpfaDetails, transform: [{ translateY: alpfaY }] }]}>
            <Svg width={lockupWidth} height={lockupWidth} viewBox="0 0 500 500">
              {ALPFA_LETTERS.map((d, index) => <Path key={`letter-${index}`} d={d} fill="#FFFFFF" />)}
              {ALPFA_LINES.map((d, index) => <Path key={`line-${index}`} d={d} fill={index === 0 ? '#FFFFFF' : '#888888'} />)}
            </Svg>
          </Animated.View>
        </View>
